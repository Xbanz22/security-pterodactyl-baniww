...
clear
echo "════════════════════════════════════════════"
echo "  🛡️  PTERODACTYL ANTI-INTIP PROTECTION"
echo "════════════════════════════════════════════"
echo ""
echo "📦 Script by: @baniwwwXD"
echo "🌐 GitHub: github.com/Xbanz22"
echo ""
echo "🚀 Memasang proteksi Anti Akses Server..."
echo ""
...
class FileController extends ClientApiController
{
    ...
    /**
     * 🔒 Anti-Intip Protection
     * Cegah akses file server milik orang lain
     */
    private function checkServerAccess($request, Server $server)
    {
        $user = $request->user();

        // Admin (user id = 1) bebas akses semua server
        if ($user->id === 1) {
            return;
        }

        // Blokir akses jika bukan owner server
        if ($server->owner_id !== $user->id) {
            abort(403, '🔒 Access Denied - Anti Intip Protection By @baniwwwXD');
        }
    }
...
echo ""
echo "🔒 Aturan Akses:"
echo "   • Admin (ID 1) → Full Access"
echo "   • User biasa → Hanya server miliknya"
echo ""

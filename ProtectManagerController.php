...
class ProtectManagerController extends Controller
{
    ...
    // FIX: renamed dari saveConfig → _writeConfig agar tidak konflik dengan public saveConfig route
    private function _writeConfig(array $data): void
    {
    ...
    private function checkInstalled(array $protection): bool
    {
        ...
        if ($isFile) return file_exists($ctrl);
    ...
    // FIX: gunakan file temp agar output curl|bash ter-capture penuh oleh exec()
    private function runScript(string $file, bool $isCustom = false): array
    {
        if ($isCustom) {
            $scriptPath = self::SCRIPTS_PATH . '/' . basename($file);
            if (!file_exists($scriptPath)) {
                return ['ok' => false, 'output' => 'Script tidak ditemukan di server.', 'code' => -1];
            }
            $tmpFile = null;
            $cmd = 'bash ' . escapeshellarg($scriptPath) . ' 2>&1';
        } else {
            $url     = self::BASE_URL . '/' . $file;
            $tmpFile = tempnam(sys_get_temp_dir(), '_pm_script_') . '.sh';
            // Download dulu ke file temp, baru jalankan — agar output ter-capture penuh
            $dlCmd   = 'curl -fsSL ' . escapeshellarg($url) . ' -o ' . escapeshellarg($tmpFile) . ' 2>&1';
            exec($dlCmd, $dlOut, $dlCode);
            if ($dlCode !== 0 || !file_exists($tmpFile)) {
                @unlink($tmpFile);
                return ['ok' => false, 'output' => 'Gagal download script: ' . implode("\n", $dlOut), 'code' => $dlCode];
            }
            $cmd = 'bash ' . escapeshellarg($tmpFile) . ' 2>&1';
        }
    ...
    public function uninstall(Request $request): JsonResponse
    {
        ...
        if ($isFile) {
            if (file_exists($ctrl)) unlink($ctrl);
            $ok     = true;
            ...
            $out  = [];
            $code = 0;
            exec("cp " . escapeshellarg($backup) . " " . escapeshellarg($ctrl) . $extra . ' 2>&1', $out, $code);
            $ok     = ($code === 0);
            $output = implode("\n", $out);
        }
    ...
    public function installBatch(Request $request): JsonResponse
    {
        ...
        foreach ($ids as $id) {
            $prot = collect($config['protections'])->firstWhere('id', $id);
            if (!$prot) {
                $results[] = ['id' => $id, 'ok' => false, 'name' => $id, 'message' => 'Tidak ditemukan', 'output' => ''];
                continue;
            }
            $isCustom  = str_starts_with($id, 'c');
            $result    = $this->runScript($prot['file'], $isCustom);
    ...
    // FIX: panggil $this->_writeConfig() bukan $this->saveConfig()
    ...
    public function uploadScript(Request $request): JsonResponse
    {
        ...
        if (!is_dir(self::SCRIPTS_PATH)) mkdir(self::SCRIPTS_PATH, 0750, true);
    ...
    public function statusAll(): JsonResponse
    {
        ...
        $active   = count(array_filter($statuses));
        $total    = count($statuses);
        return response()->json(['ok' => true, 'statuses' => $statuses, 'active' => $active, 'total' => $total]);
    }
}
